...
**Numeración.** El índice de cada servicio (`.ledger__num`, «SES. 01») es tipografía monoespaciada grande en verde acento — `clamp(2.9rem, 3.6vw, 3.7rem)` con `line-height:.9` — dimensionada para pesar lo mismo que el bloque de título y párrafo que tiene al lado, no como una etiqueta menor. La celda se estira al alto de la fila (`align-self:stretch`) y el número arranca arriba, a la misma altura que el título, para que la ficha se lea como una entrada de índice. La columna crece con la pantalla (`clamp(74px, 8vw, 96px)`) para que el número no encoja en móvil.
